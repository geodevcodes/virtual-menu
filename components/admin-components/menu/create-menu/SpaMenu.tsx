...
interface SpaMenuProps {
  ...
  isSpaOpen: boolean;
  setIsSpaOpen: React.Dispatch<React.SetStateAction<boolean>>;
  register: any;
  errors: any;
}
...
export default function SpaMenu({
  ...
  const spaCategories = [{ id: "spaMenuFile", label: "Spa Menu" }];
  ...
  return (
    <div className="mb-5 border-b border-gray-100 pb-3">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-700">Spa Menu</h3>
        <button
          type="button"
          onClick={() => setIsSpaOpen((s) => !s)}
          className="cursor-pointer p-1 rounded hover:bg-gray-100 transition-colors"
        >
          <ChevronRight
            size={18}
            className={`transform transition-transform ${
              isSpaOpen ? "rotate-90" : "rotate-0"
            }`}
          />
        </button>
      </div>
            ...
            <div className="my-4">
              ...
              <input
                ...
                className={`w-full py-3 px-4 text-sm text-[#667085] rounded-lg border bg-white ${
                  errors.spaReviewLinkUrl ? "border-red-500" : "border-gray-300"
                } outline-none`}
              />
              {errors.spaReviewLinkUrl && (
                <p className="text-red-500 text-xs mt-1">
                  {errors.spaReviewLinkUrl.message}
                </p>
              )}
              <Link
